...
## Local dev notes for this repo

This fork includes:

- Local **OpenSearch** via `docker-compose.yml`
- A unified search endpoint at `GET /api/search?q=...` (gracefully falls back to Fuse when OpenSearch is unavailable)
- Protected reindex endpoints:
  - `POST /api/opensearch/reindex/blog`
  - `POST /api/opensearch/reindex/pokemon?limit=151`
- A Sanity webhook receiver:
  - `POST /api/opensearch/webhook/sanity`

### Environment variables

Copy `apps/web/.env.example` to `apps/web/.env.local` and fill in required values.

For OpenSearch + webhook/reindex you’ll need at minimum:

- `OPENSEARCH_NODE` (e.g. `http://localhost:9200`)
- `REINDEX_TOKEN` (used as `Authorization: Bearer <token>`)
- `SANITY_WEBHOOK_SECRET` (used as `x-webhook-secret: <secret>` or `x-webhook-signature: sha256=<hmac>`)
...
Then (after the web app is running), bulk index content:

```bash
curl -X POST http://localhost:3000/api/opensearch/reindex/blog -H "Authorization: Bearer $REINDEX_TOKEN"
curl -X POST "http://localhost:3000/api/opensearch/reindex/pokemon?limit=151" -H "Authorization: Bearer $REINDEX_TOKEN"
```
